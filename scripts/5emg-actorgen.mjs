...
export async function mgNew5eActor(formData) {
	...
	if (crValue.includes("/")) {
		mgUtils.assignToObject(dummy, "system.attributes.cr", mgUtils.parseFraction(crValue));
	} else {
		mgUtils.assignToObject(dummy, "system.attributes.cr", parseInt(crValue));
	}
	...
	//If ability string was pasted, assign abilities, if not, initialize object in 0s
	if (formData.abilityString.value != '') {
		...
		const abilities = ["strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma"];
		for (let i = 0; i<6; i++) {
			mgUtils.assignToObject(dummy, `system.abilities.${abilities[i]}.mod`,0);
		}
	}
	...
	if (selStats.atks > 1) {
		await newActor.createEmbeddedDocuments("Item", [{
			name: "Multiattack", type: "feature",
			system: {
				description: { value: `The ${newActor.name} makes ${selStats.atks} attacks.` },
	...
	await newActor.createEmbeddedDocuments("Item", [{
		name: "Attack", type: "feature",
		system: {
			description: { value: `<em>Melee or Ranged Weapon Attack:</em> [[/attack]] reach 5 ft. or range 60 ft., one target. <em>Hit:</em> [[/damage average]] damage` },
